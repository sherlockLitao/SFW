...
class comp_h
{
    ...
    public:
      ...
      comp_h (int current_axis11, bool sign11, const Eigen::ArrayXXd & X11): current_axis1(current_axis11), sign1(sign11), X1(X11){}
        ...
        for(i=0; i<_d; i++){
          ...
          for(j=0; j<pow(2,i); j++){
            ...
            if(split_index[jj-current_step] >= split_index[jj+current_step])split_index[jj] = split_index[jj - current_step];
              ...
              comp_h cmp(current_axis, sign, X);
...
class comp_m
{
    ...
    public:
      ...
      comp_m (int current_axis11, const Eigen::ArrayXXd & X11): current_axis1(current_axis11), X1(X11){}
      
      bool operator()(const int &i1, const int &i2)
      {
        return ( X1(i1,current_axis1) <  X1(i2,current_axis1) );
      }
...
//using recursive sort algorithm
class Morton_sort
{
  public:
  ...
  public:
    ...
    Morton_sort (const Eigen::ArrayXXd & X1):X(X1){}

    void morton_median_sort_d(std::vector<ptrdiff_t>::iterator begin, std::vector<ptrdiff_t>::iterator end, int first_axis){
        ...
        // perform recursive sort
        for(i=0; i<pow_d; i=i+1){
          morton_median_sort_d(split_index[i], split_index[i+1], 0);
        }

    }

    void Morton_sort_median_d(std::vector<ptrdiff_t>::iterator begin, std::vector<ptrdiff_t>::iterator end)
    {
      n = X.rows() * 2;
      d = X.cols();
      pow_d = 1;
      ...
      morton_median_sort_d (begin, end, 0);
    }
...
//Morton order
Eigen::ArrayXi Morton_Curve_Order(const Eigen::ArrayXXd & X)
{
    ...
    Morton_sort Ms(X);
    Ms.Morton_sort_median_d(indices.begin(),indices.end());
...
PYBIND11_MODULE(base, m)
{
    ...
    m.def("morton_order", &Morton_Curve_Order);
