...
class Encoder(nn.Module):
    def __init__(self, num_filter, d=48, poincare_output=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # device = out.device
        # x = torch.zeros(out.shape[1], device=device)

        if self.poincare_output:
            # return exp0_poincare(out, x)
            return exp0_poincare(out)
        else:
            return out
    

class Decoder(nn.Module):
    def __init__(self, num_filter, bottleneck_size=48, poincare_output=False):
        super().__init__()
        self.num_filter = num_filter
    ...
    def forward(self, x):
        ...
        if self.poincare_output:
            out = log0_poincare(x)
            out = torch.relu(self.fc5(self.fc4(out)))
        else:
...
class AE(nn.Module):
    def __init__(self, input_shape, d=48, poincare_output=False):
        super().__init__()
        self.encoder = Encoder(input_shape, d, poincare_output)
        self.decoder = Decoder(input_shape, d, poincare_output)
        self.d = d
