...
def dist_poincare(x, y, r=1):
    ...
    frac = num/denom.clamp_min(MIN_NORM)
    ### 1+2*r*frac may bring numerical error
    return torch.arccosh(1+2*r*frac.clamp_min(MIN_NORM))/np.sqrt(r)
...
def expMap(u, x, stable=False):
    ...
    if stable:
        norm_u = minkowski_ip(u,u)
        norm_u = (norm_u.clamp_min(MIN_NORM))**(1/2)
        y = torch.cosh( torch.clamp(norm_u, min=-10, max=10))*x + torch.sinh(torch.clamp(norm_u, min=-10, max=10)) * u/norm_u
    else:
...
def exp_poincare(v, x):
    ...
    normalized_v = v/torch.clamp(norm_v, min=MIN_NORM)
...
def exp0_poincare(v):
    """
        Project v\in T_0 B(=R^d) to the Poincaré ball
    """
    norm_v = torch.norm(v, dim=-1, keepdim=True)
    # th = torch.tanh(norm_v)
    th = torch.clamp(torch.tanh(norm_v), min=-1+MIN_NORM, max=1-MIN_NORM)
    normalized_v = v/torch.clamp(norm_v, min=MIN_NORM)
   
    return th*normalized_v

def log0_poincare(v):
    """
        Project v\in the Poincaré ball to T_0 B
    """
    norm_v = torch.norm(v, dim=-1, keepdim=True)
    th = torch.arctanh(torch.clamp(norm_v, min=-1+MIN_NORM, max=1-MIN_NORM))
    normalized_v = v/torch.clamp(norm_v, min=MIN_NORM)
