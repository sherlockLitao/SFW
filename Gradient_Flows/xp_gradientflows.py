...
if __name__ == "__main__":
    ...
    for k in range(n_try):
        ...
        for e in range(n_epochs):
            ...
            x_hsw = expMap(-lr*proj_hsw, x_hsw)


            # gen = torch.Generator(device=device)
            # gen.manual_seed(args.seed)
            # sw = ot.sliced_wasserstein_distance(x_sw, X_target, n_projections=num_projections, seed=gen)
            ...
            z = (1-norm_x**2)**2/4
            x_swp = exp_poincare(-lr * z * grad_x0_swp, x_swp)


            sfw = SFW(x_sfw, lorentz_to_poincare(X_target), hyperbolic_model="Poincare", spf_curve='Hm_p', k=5, p=2, eps=1e-5)**2
            grad_x0_sfw = torch.autograd.grad(sfw, x_sfw)[0]
            norm_x = torch.norm(x_sfw, dim=-1, keepdim=True)
            z = (1-norm_x**2)**2/4
            x_sfw = exp_poincare(-lr * z * grad_x0_sfw, x_sfw)
            ...
            a = torch.ones((n,), device=device, dtype=torch.float64)/n
            b = torch.ones((n,), device=device, dtype=torch.float64)/n
            ...
            if torch.any(torch.isnan(x_sfw)):
                L_sfw[k, e] = np.inf
            else:
                M_sfw = torch.arccosh(torch.clamp(-minkowski_ip2(x_test, poincare_to_lorentz(x_sfw)), min=1+1e-15))**2
                w_sfw = ot.emd2(a, b, M_sfw)
                L_sfw[k, e] = w_sfw.item()
    ...
    np.savetxt("./Results/sfw_loss_"+args.type_target+"_"+args.target, L_sfw)
